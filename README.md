...
```
notebook/
├── index.html                ← the website (Notes/Paper, Doubt Chat, Flashcards, Quiz)
├── vercel.json                ← server timeout config (60s, Vercel's max on free plan)
├── lib/
│   ├── providers.js              ← shared module that calls every AI provider + fallback chain
│   └── _limits.js                 ← shared rate-limit / premium / Google-auth helpers
├── api/                        ← every file here IS a real Vercel serverless function —
│   │                             see "Why lib/ exists" below for why that matters
│   ├── generate.js                ← multi-agent notes/paper/project backend
│   ├── chat.js                     ← doubt-solving chat backend
│   ├── study-tools.js               ← flashcards & quiz backend
│   ├── sync.js                       ← cloud sync for signed-in students
│   ├── auth.js                        ← verifies a Google sign-in
│   ├── config.js                       ← serves the public Google Client ID
...
### Why `lib/` exists (important — read this before adding new files)

**Vercel's Hobby (free) plan hard-caps a deployment at 12 serverless
functions, and it treats *every* `.js` file directly under `/api/` as a
separate function** — including plain shared helper modules that don't
even export a route handler. `providers.js` and `_limits.js` are used by
almost every endpoint but aren't endpoints themselves, so keeping them in
`/api/` was silently burning 2 of the 12 function slots for nothing. Once
enough real endpoints were added on top of that, the deployment could
fail entirely — taking down the *whole* site, not just the newest
feature, since Vercel refuses to deploy at all past the limit. Moving
these two files to `lib/` (which Vercel's routing convention ignores)
fixed that: **the project now uses 8 real functions**, with real headroom
before hitting 12 again.

**If you add another backend file that ISN'T meant to be called directly
over HTTP (another shared helper), put it in `lib/`, not `api/`.** Only
put a file in `api/` when it's a genuine endpoint you want reachable at
`/api/<filename>`.

## Two separate modes

The app now has a mode switch at the top of the chat with four modes:

- **📝 Notes/Paper** — the multi-agent pipeline: plans a document
...
- **💬 Doubt Chat** — a plain, stateful conversation for quick questions.
  It NEVER touches the plan/section/PDF pipeline.
- **🗂️ Flashcards** — turns a topic into a deck of front/back flashcards
  (click to flip), downloadable as a print-ready PDF grid.
- **❓ Quiz** — turns a topic into an interactive multiple-choice quiz with
  immediate right/wrong feedback, an explanation per question, and a
  running score.

Flashcards and quiz share the SAME daily quota as notes/paper/project
generation (`api/study-tools.js` uses the same `nb` counter prefix as
`api/generate.js`) — they're a lighter-weight sibling pipeline, not a
separate allowance.

## Multi-provider AI with automatic fallback

`lib/providers.js` is the one place that knows how to call every AI
...
## Flashcards & Quiz (api/study-tools.js)

A lighter-weight sibling to the notes/paper pipeline, since the output
shape is completely different (structured cards/questions, not
heading+content sections). Uses the same reliability techniques proven
out in `generate.js` — chunked generation (a handful of cards/questions
per call, not the whole deck at once) and verifying the actual count came
back right — but is intentionally simpler: 3 attempts per chunk, no judge
agent, no vision check. A deck of flashcards is lower-stakes than a full
document, so this is a fast, "good enough" bar rather than
`generate.js`'s much heavier guarantee. Chunks run in parallel (each told
its own batch index and asked to cover a different angle, to minimize
overlap) rather than sequentially, to stay well within Vercel's 60-second
function limit even for a 40-item deck. Shares the SAME daily quota as
notes/paper/project generation.

## Cloud sync (signed-in students only)

Once signed in with Google, chats sync via `api/sync.js` — GET restores
them on a new device, POST saves the current set — stored in the SAME
Upstash database already used for rate limiting, just a different key
prefix (`nb-sync:<email>`), no new infrastructure needed. This is
deliberately simple last-write-wins sync: on sign-in, local and cloud
chats are merged by id (keeping whichever version has more messages, as a
"more complete" heuristic), and every local save is pushed to the cloud
(debounced 1.5s). Real conflict resolution across simultaneously-active
devices is out of scope — for a student using their own notes across a
couple of devices, that's a reasonable, honest tradeoff.

**Switching Google accounts**: the "Switch" button next to a signed-in
student's name calls Google's `disableAutoSelect()` before showing the
sign-in button again, so the account chooser (including "use another
account") appears instead of silently re-signing in as the same person.

## Search, tags, and streaks (local, no backend needed)

- The sidebar has a search box that filters chats by title, tag, or
  message content.
- Any chat can be tagged (🏷️ button) — e.g. by subject — and tags are
  searchable too.
- A 🔥 streak badge tracks consecutive days with at least one generation,
  chat message, flashcard deck, or quiz — computed entirely from a local
  list of active dates, no server involved.

## Voice input

A mic button appears next to the composer in browsers that support the
Web Speech API (Chrome, Edge — not Safari or Firefox, where the button
just stays hidden rather than showing something that would fail). Tapping
it transcribes speech directly into the input box.
...
3. **Environment variables** (Vercel → Settings → Environment Variables):
   ...
   - `SESSION_SECRET` — required if `GOOGLE_CLIENT_ID` is set. Any long
     random string (e.g. run `openssl rand -hex 32` and paste the output).
     Used to sign the app's own 30-day session token, so a signed-in
     student stays signed in — without this, the app falls back to
     Google's raw ~1hr ID token as the session, which is what caused
     signed-in students to look "signed out" within the same day.
   - `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN` — optional, for
     ...
     bypass and the 120B cap is skipped entirely. Also required for the
     credit system (3/day + 10 sign-up bonus) — without it, generation
     stays open/unmetered rather than blocking anyone.
   - `ADMIN_SECRET` — only needed for redeem.js/feedback.js's admin actions
   - `SERPER_API_KEY` — optional, from serper.dev (free tier, no card
     needed). When set, a specimen/exam paper request first does a real
     web search for that exact board/class/subject's current format
     before planning the paper, instead of relying only on the patterns
     built into the prompt. Without it, paper planning still works exactly
     as before — this is a pure add-on.
   - `RAZORPAY_KEY_ID` / `RAZORPAY_KEY_SECRET` — required for the Basic
     (₹99) / Pro (₹199) / Premium (₹299) plan purchase buttons on the
     Premium page. From Razorpay Dashboard → Settings → API Keys. Without
     these, those buttons show a clear "payments not configured" message
     — the manual UPI QR + redeem-code flow on the same page keeps
     working either way.
   - `REQUIRE_SIGN_IN` — set to the literal string `true` to make signing
     in with Google mandatory (once GOOGLE_CLIENT_ID's origins are
     confirmed working — see the Google Sign-In section below). Leave
     unset/false to keep the app open to anonymous/IP-based students while
     sign-in stays available as an optional bonus (credits sync, +10
     sign-up bonus).
   - `*_API_KEY_2` / `*_API_KEY_3` — optional, for any provider (e.g.
     `GROQ_API_KEY_2`, `MISTRAL_API_KEY_2`). If set, that provider's calls
     round-robin across all configured keys instead of hammering one.
   - `MISTRAL_API_KEY` / `HUGGINGFACE_API_KEY` — optional, adds those two
     providers into the fallback chains. Without them, those two links in
     the chain are skipped automatically (same as any other missing key).
...
## Google Sign-In (identity only — mandatory once configured)
...
this code** (the secret is only needed for a full OAuth code-exchange flow
that reads/writes a user's data on Google's APIs on their behalf — a
plain "who is this person" sign-in like this doesn't do that exchange at
all).

**Once `GOOGLE_CLIENT_ID` is set, signing in becomes mandatory** — not
just a nicety. Both `api/generate.js` and `api/chat.js` reject any
request without a verified Google identity with a `401` and
`requiresSignIn: true` once that env var is present. This is enforced
**server-side**, not just by hiding a button on the frontend — hiding a
button doesn't stop someone from calling the API directly, so the real
gate lives in the API itself. The frontend also shows a blocking "sign in
to continue" panel over the composer, disables the input, and reacts to a
`401`/`requiresSignIn` response even if that panel somehow didn't catch
it first (e.g. `/api/config` hadn't loaded yet).

Before `GOOGLE_CLIENT_ID` is set, the app stays fully open (current
IP-based limits) — there'd be no way to sign in at all, so blocking
everyone would just break the app with no way to fix it without also
configuring sign-in.
...
add the Client Secret anywhere — it isn't used. In Google Cloud Console →
Credentials, make sure your deployed domain is listed under "Authorized
JavaScript origins" for that OAuth Client ID, or the button won't work.
