...
// Sum ≈ 700+700+300+600+200+400 = 2900 nominal ceiling — the real daily
// total in practice lands close to ~2500 because chains fail over (a
// section that succeeds on the first provider never touches the rest),
// so most days nowhere near every provider's full budget gets spent.
...
async function withDailyBudget(providerName, callFn){
  ...
  if (!limit) return callFn();
  const limitResult = await checkAndIncrementLimit('global', limit, `nb-budget-${providerName}`);
  if (!limitResult.allowed) {
    throw new Error(`${providerName} daily call budget reached (${limit}/day) — protecting the shared key, falling back to the next provider.`);
  }
  return callFn();
}

const PROVIDER_CALLERS = {
  openrouter: (messages, maxTokens, origin, timeoutMs) => withDailyBudget('openrouter', () => callOpenRouter(process.env.OPENROUTER_API_KEY, messages, origin, MODELS.openrouter, maxTokens, timeoutMs)),
  openrouterFallback: (messages, maxTokens, origin, timeoutMs) => withDailyBudget('openrouterFallback', () => callOpenRouter(process.env.OPENROUTER_API_KEY, messages, origin, MODELS.openrouterFallback, maxTokens, timeoutMs)),
  ...
  gemini: (messages, maxTokens, origin, timeoutMs) => withDailyBudget('gemini', () => callGemini(process.env.GEMINI_API_KEY, messages, maxTokens, undefined, timeoutMs)),
  grok: (messages, maxTokens, origin, timeoutMs) => withDailyBudget('grok', () => callGrok(process.env.XAI_API_KEY, messages, maxTokens, undefined, timeoutMs)),
  nvidia: (messages, maxTokens, origin, timeoutMs) => withDailyBudget('nvidia', () => callNvidia(process.env.NVIDIA_API_KEY, messages, maxTokens, undefined, timeoutMs)),
};
