...
import { callWithFallback, callGeminiVision } from '../lib/providers.js';
import { checkPremiumToken, checkAndIncrementLimit, spendCredit, getCreditBalance, resolveIdentity, PLANS } from '../lib/_limits.js';

// ---- Optional: real exam-format search before planning a paper ----
//
// Not active by default — needs a SERPER_API_KEY (free tier at
// serper.dev, no credit card, a few thousand free searches/month) set as
// a Vercel environment variable. Without it, this returns '' immediately
// and plan generation falls back to the board/subject patterns already
// written into PLAN_PROMPT, exactly as before — this is a pure add-on,
// never a requirement.
async function searchPaperFormatIfConfigured(userRequest){
  ...
  try {
    const res = await fetch('https://google.serper.dev/search', {
      ...
      body: JSON.stringify({ q: `${userRequest} exam paper format sections marks structure 2026` }),
    });
    if (!res.ok) return '';
    const data = await res.json();
    const results = (data.organic || []).slice(0, 4);
    if (results.length === 0) return '';
    const snippets = results.map(r => `- ${r.title}: ${(r.snippet || '').slice(0, 200)}`).join('\n');
    return `\n\n(Current web search results on this exam's real format — ground your markScheme in these if they're relevant and specific, rather than guessing:\n${snippets}\n)`;
  } catch (err) {
    return ''; // best-effort only — a search failure must never block plan generation
  }
}

const PREMIUM_DAILY_LIMIT = 10; // premium still uses a simple flat daily cap, not the free-tier credit system
...
//
// THREE tiers now, matching the plan system in lib/_limits.js (PLANS —
// free="low", basic/pro="mid", premium="high"). Each tier's chain leads
// with the provider/model that tier is actually meant to use (see
// MODEL_TIERS in lib/providers.js for which model each provider serves
// per tier), then falls back through the rest so a single provider
// outage never fully blocks a tier.
//
// The QUALITY chain is separate and NOT tier-based — judge/visual-judge
// passes and Answer Key sections always lead with Gemini (the protected
// reserve) regardless of the student's plan, because a mismatched answer
// key or an unverified section is exactly the kind of "wrong content" a
// paying AND a free student both deserve to be protected from.
const LOW_TIER_CHAIN = ['groq', 'huggingface', 'openrouter', 'nvidia', 'openrouterFallback'];
const MID_TIER_CHAIN = ['groq', 'gemini', 'mistral', 'openrouter', 'nvidia', 'openrouterFallback'];
const HIGH_TIER_CHAIN = ['gemini', 'groq', 'mistral', 'grok', 'nvidia', 'openrouterFallback'];
const QUALITY_PROVIDER_CHAIN = ['gemini', 'grok', 'groq', 'openrouter', 'nvidia', 'openrouterFallback'];
const CHAIN_BY_TIER = { low: LOW_TIER_CHAIN, mid: MID_TIER_CHAIN, high: HIGH_TIER_CHAIN };
const JUDGE_TOKENS = 2000;
const VISUAL_JUDGE_TOKENS = 700;
const SUMMARY_TOKENS = 350;

// The "vision" check: looks at an actual SCREENSHOT of the rendered
// document (sent by the frontend after it builds the PDF preview) instead
// of the raw text — this is what catches things a text-only check
// structurally can't: a section that renders visibly blank even though
// its JSON technically "had content", a diagram overlapping text, a
// table that's obviously cut off at a page edge. Only Gemini has a vision
// call wired up here (see providers.js callGeminiVision) — if
// GEMINI_API_KEY isn't set, this mode is skipped entirely by the
// frontend rather than failing the document.
const VISUAL_JUDGE_PROMPT = `You are Notebook's visual quality inspector. You are looking at a SCREENSHOT of a study document/paper that was just generated, exactly as a human would see it on the page.

Look at the image and check ONLY things a screenshot can show that plain text can't:
- Is any section visibly BLANK, cut short, or does it show a "couldn't be generated" placeholder message?
- Does any diagram overlap text, render broken/garbled, or look like an empty box?
- Is there a large empty gap that looks like missing content, rather than normal spacing?
- Does any table or list look visibly cut off?

Do NOT comment on writing style, wording choices, or anything you'd need to read carefully to judge — this is a QUICK VISUAL scan, not a content review (a separate text-based check already happened for that).

Return ONLY valid JSON, no commentary:
{ "ok": true or false, "note": "if not ok, one short specific sentence describing exactly what looks visibly wrong and roughly where (e.g. section heading nearby)" }`;
...
// A short, on-demand "TL;DR" for an already-generated document (the
// "Summary" button in the frontend) — deliberately NOT part of the main
// plan/section pipeline, so it never costs the student an extra daily
// generation: the frontend always calls this with isContinuation:true,
// the same way it already treats judge/visual-judge calls as part of the
// one document the student already paid a quota slot for.
const SUMMARY_PROMPT = `You are Notebook's summarizing agent. You are given the full text of a study document that has already been written. Produce a short "TL;DR" a student can read in 15 seconds before diving into the full document.

Return ONLY valid JSON, no commentary:
{ "summary": "3 to 5 short bullet-style lines, each starting on its own line with a dash, covering only the genuinely most important takeaways — no restating the title, no filler, no generic closing line" }

Each line must be one real, specific fact or idea from the document — never a vague line like "This document covers various important topics."`;
...
HUMAN VOICE — this is checked, not a suggestion: the content must read like it was actually written by a knowledgeable human teacher, not generated. Concretely:
- Vary sentence length on purpose — mix short, punchy lines with longer ones; never a whole section of same-length sentences in a row (that's the single biggest tell of AI writing).
- NEVER use these overused AI-tic words/phrases anywhere: "delve into", "dive into", "in today's world/fast-paced world", "it's important to note that", "plays a crucial/vital role", "a testament to", "in conclusion" / "in summary" (just stop when you're done, don't announce it), "moreover", "furthermore" (as sentence-openers), "unlock", "unleash", "landscape" (as in "the landscape of X"), "robust", "seamless", "boasts", "underscores", "tapestry", "realm", "navigate" (as in "navigate the complexities of"), "myriad", "plethora", "at the end of the day", "when it comes to X".
- Don't structure every single point the same templated way ("X is important because Y. This means Z.") — real teachers explain some things in one line, others take three, some as a quick aside, some with a rhetorical question.
- State things directly and with real opinion/confidence where a teacher genuinely would ("this is the part everyone gets wrong", "the key trick here is...") rather than neutral, hedge-everything AI phrasing.
- No generic scene-setting before getting to the actual point ("In the world of chemistry, reactions are fundamental...") — start with substance immediately.
...
const PLAN_PROMPT = `You are Notebook's planning agent. A student has made a request for study notes, a specimen/exam paper, a school PROJECT REPORT, or an EBOOK. Your ONLY job right now is to decide the document's structure — NOT write any content yet.

Return ONLY valid JSON, no commentary, matching exactly:
{
  "type": "notes" or "specimen" or "project" or "ebook",
  "title": "a clear title for the whole document",
  "markScheme": "for a specimen paper: one precise paragraph stating the EXACT structure — e.g. 'Section A: 5 MCQs, 1 mark each = 5 marks. Section B: 4 VSA, 2 marks each = 8 marks. Section C: 3 SA, 3 marks each = 9 marks. Section D: 2 LA, 4 marks each = 8 marks. Total = 30 marks, matching the requested total.' For notes/project/ebook, leave this as an empty string.",
  "sections": [
    { "heading": "Section heading", "questionCount": 0 }
  ]
}

"questionCount" = how many numbered questions THIS section contains (0 for non-question sections like an overview, notes topic, project section, or ebook chapter).

WHICH type to pick: "project" is for "make me a project on X" / "school project report" / "project file for [subject]" style requests — a complete school project REPORT document, not exam notes or a test paper. "ebook" is for "write an ebook/book on X" / "write a guide to X" / requests that name a number of chapters or ask for something book-length and narrative rather than exam-focused. If the request just asks to explain/summarize/revise a topic, that's "notes"; if it asks for a test/exam/MCQs, that's "specimen".

If type is "ebook", the "sections" array (used as CHAPTERS here) MUST be, in this exact order:
1. { "heading": "Introduction", "questionCount": 0 } — why this book/topic matters, who it's for, what the reader will be able to do by the end.
2. Then as many CHAPTER sections as the subject genuinely needs — each a distinct, logically-ordered chapter of the book (e.g. for "ebook on personal finance for beginners": "Understanding Income and Expenses", "Building an Emergency Fund", "Getting Started with Investing", "Common Mistakes to Avoid" — headings must be specific chapter titles, never generic like "Chapter 1" or "Main Content"). If the student names a number of chapters, use exactly that many; otherwise size it to what the topic genuinely needs (a focused topic: 4-6 chapters; a broad one: more).
3. { "heading": "Conclusion", "questionCount": 0 } — ties the book together, a genuine send-off, not a bullet recap.
Do not add a Certificate/Acknowledgement/Bibliography for an ebook — those are project-report conventions, not book conventions.
...
- If this is a SPECIMEN/EXAM PAPER: base the structure on how THIS SPECIFIC board, class, and subject's REAL exams are genuinely structured — not a generic template. Work out the EXACT mark arithmetic for your chosen structure and write it precisely into "markScheme" — this exact text is shown to every section-writer so they all agree with each other and the Overview. Verified real current patterns to match (adapt the marks proportionally if the student asks for a different total, but keep the same section SHAPE):
  ...
  - ANSWER KEY IS OPT-IN, NOT DEFAULT: a real exam paper a student sits for does NOT come with the answers printed on it — only include any "Answer Key" section AT ALL if the student's request explicitly asks for one (words like "with answers", "with solutions", "answer key", "with mark scheme", "solved"). If they just asked for a paper/test/practice paper with no such wording, the plan must contain ONLY question sections — no answer key section anywhere, even a partial one. When answers genuinely ARE requested — CRITICAL: do NOT create one single "Answer Key" section for the whole paper — a full answer key in one chunk is too large to generate reliably. Instead create ONE SEPARATE answer-key section immediately after each question section, named like "Answer Key — Section A", "Answer Key — Section B", etc. Each answer-key section's questionCount should match the question section it answers.
...
- "Bibliography": a numbered list of 4-6 GENERIC, realistic source types Indian students actually cite for projects — the relevant NCERT/board textbook for this subject, Wikipedia, and 2-3 well-known general educational sites (e.g. Byju's, Vedantu) named generically by site, not fabricated specific book titles, authors, editions, or ISBNs.`;
    ...
    typeRules = `This is one CHAPTER of an ebook — real book-length narrative writing, not exam notes and not a bullet-point cram sheet. Match the heading to how it's used:
- "Introduction": hook the reader in the first couple of sentences (a question, a striking fact, a relatable scenario) — never "In this book, we will discuss...". Say plainly what the reader will be able to do/understand by the end, and why it's worth their time.
- A CHAPTER (the real body of the book): this is genuine long-form prose — paragraphs that flow into each other, real explanations with worked-through reasoning, concrete examples and mini-stories/analogies where they help understanding, not a list of disconnected facts. Write substantially more than a notes section would — a chapter should feel like a real chapter, developed and complete, not a summary of one. Use sub-headings within the chapter (as **bolded** short lines on their own) only where the chapter naturally has distinct parts — don't force notes-style fragmentation onto what should be flowing prose.
- "Conclusion": a genuine send-off that ties the book's threads together and leaves the reader with something to act on or think about — not a mechanical bullet recap of every chapter.
Depth and word count should genuinely match what a real published ebook chapter on this exact subject needs — this is the one type where being SUBSTANTIAL is correct, unlike notes (which stay deliberately tight). Hard ceiling: roughly 10 printed pages of content for this ONE chapter — develop the chapter fully within that, don't pad past it, and don't try to fit more than one chapter's worth of material into a single chapter just because the topic is big (a broad topic should become MORE chapters in the plan, not one overlong one).`;
  } else {
...
- STRUCTURE, not prose: break the section into clearly separate sub-points, each on its own line, starting with a short **bolded label** (the term/sub-concept name) followed by a colon and ONE crisp line of explanation — like a real notes page, never a wall of paragraph text. Never merge two distinct ideas into one run-on sentence.
- DENSITY LIMIT: cover only the genuinely essential sub-points of this heading — as a rough discipline, aim for roughly 4-7 sub-points per section (fewer for a narrow heading, never padded up to hit a number). If a topic truly needs more, split it across sub-points rather than writing longer paragraphs per point. Quality and precision over quantity of words.
...
- Never add throat-clearing, restating the heading back as a sentence, or a generic closing line ("In conclusion...", "This shows the importance of...") — start directly with the first real point and stop the moment the last real point is covered.
...
- "explanation" field for a notes section: leave it as an EMPTY string "" — the heading already says what the section covers, restating it in a subtitle line is clutter on a printed notes page.`;
  }
...
DIAGRAM rules: include a "diagram" whenever the section is ABOUT something a real teacher would draw — ray/light diagrams (refraction, reflection, lenses), circuit diagrams, geometry figures (triangles, circles with labelled parts), force/vector diagrams, labelled biological structures/cycles, process flowcharts, a labelled map-style sketch, OR A GRAPH. If the content mentions "draw a diagram/graph" or describes a physical setup, cycle, process, or any quantitative relationship between two variables (e.g. distance vs time, velocity vs time, pressure vs volume, population vs year, a titration curve), you MUST include one — don't just describe it in words and skip the visual. Make it a REAL, clear, well-labelled diagram, not a token decoration — use the full canvas, label every part, and use arrows/polygons to show direction or flow where relevant.

GRAPH rule (bar/line/plotted-curve graphs specifically): a graph must be a REAL plotted graph, not a decorative sketch — always include: (1) a horizontal x-axis and vertical y-axis drawn as <line> elements meeting at an origin, (2) 3-6 evenly-spaced tick marks per axis with numeric or category <text> labels, (3) an axis title in <text> near each axis (e.g. "Time (s)", "Velocity (m/s)"), (4) the actual data as either a <polyline>/<path> line (for a trend/curve) or evenly-spaced <rect> bars (for a bar chart), drawn to genuinely reflect the real relationship being taught (a straight line for uniform motion, a curve for accelerating motion, correct relative bar heights for real comparative data) — never a random or meaningless shape, (5) light grid lines (thin, low-opacity <line> elements) behind the data so values can be read off the axes. <polyline points="x1,y1 x2,y2 ..."> is allowed in addition to the tags listed below specifically for graphs.

Format requirements (follow EXACTLY, this is validated and stripped if wrong):
- Must start with <svg viewBox="0 0 500 320"> and end with </svg> — no width/height attributes on the svg tag itself.
- Only these child tags: <line> <path> <circle> <rect> <polygon> <polyline> <text> <g>. No <script>, no event handler attributes (onclick etc), no <image>, no external references, no <marker>/<defs>.
...
export default async function handler(req, res) {
  ...
  const { mode, userRequest, sectionHeading, type, isContinuation, startingQuestionNumber, questionCount, markScheme, premiumToken, continueFrom, sections, imageBase64 } = req.body || {};
  if (mode !== 'plan' && mode !== 'section' && mode !== 'judge' && mode !== 'visual-judge' && mode !== 'summary') {
    return res.status(400).json({ error: 'Request must include mode: "plan", "section", "judge", "visual-judge", or "summary".' });
  }
  if (mode !== 'judge' && mode !== 'visual-judge' && !userRequest) {
  ...
  if (mode === 'visual-judge' && !imageBase64) {
    return res.status(400).json({ error: 'Visual-judge mode requires "imageBase64".' });
  }

  // The daily limit counts once per document (the plan call). All section,
  // judge, AND visual-judge calls that follow it are part of the SAME
  // user-initiated generation — only the very first "plan" call counts
  // against the quota. "tier" (low/mid/high — see PLANS in lib/_limits.js)
  // is resolved on EVERY call though, metered or not, because every
  // section of a document should use the SAME model tier the student's
  // plan entitles them to, not just the first "plan" call.
  let tier = 'low';
  if (!isContinuation) {
    // Once GOOGLE_CLIENT_ID is configured, signing in becomes MANDATORY —
    // not just a nicety for cross-device quota. This is enforced HERE,
    // server-side, not just by hiding a button on the frontend: hiding a
    // button doesn't stop someone from calling this endpoint directly, so
    // the actual gate has to live in the API itself. Before
    // GOOGLE_CLIENT_ID is set, the app stays open (there'd be no way to
    // sign in at all, so blocking everyone would just break the app with
    // no way to fix it without also configuring sign-in).
    const { key: identityKey, user } = await resolveIdentity(req, {
      sessionToken: req.body?.sessionToken,
      googleIdToken: req.body?.googleIdToken, // still accepted for old clients that haven't picked up a session token yet
    });
    if (process.env.REQUIRE_SIGN_IN === 'true' && !user) {
    ...
    if (isPremium) {
      const limitResult = await checkAndIncrementLimit(identityKey, PREMIUM_DAILY_LIMIT, 'nb');
      if (!limitResult.allowed) {
        return res.status(429).json({ error: `Daily limit reached (${PREMIUM_DAILY_LIMIT} per day). Try again after midnight.` });
      }
      tier = 'high'; // the legacy UPI+redeem-code premium path maps to the top model tier
    } else {
      const spend = await spendCredit(identityKey);
      if (!spend.allowed) {
        return res.status(429).json({ error: 'Out of credits for today — credits top up daily, or upgrade to Premium for more.', outOfCredits: true });
      }
      tier = PLANS[spend.plan]?.tier || 'low';
    }
  } else {
    // Continuation call (section/judge/summary/visual-judge) — this is
    // free (no credit spend, see above), but we still need to know which
    // tier to route it to, so re-resolve identity and READ (never spend)
    // the plan. A stateless read like this is cheap even at high volume.
    try {
      const { key: identityKey } = await resolveIdentity(req, {
        sessionToken: req.body?.sessionToken,
      ...
      if (isPremium) {
        ...
        tier = PLANS[plan]?.tier || 'low';
      }
    } catch (err) {
      tier = 'low'; // never let tier resolution itself block a continuation call
    }
  }

  // Visual-judge is a completely separate path — it calls Gemini's vision
  // endpoint directly instead of the text-only provider chain, since none
  // of the other providers have image support wired up here. If Gemini
  // isn't configured, this fails fast with a clear error the frontend
  // treats as "skip the visual check", never as a reason to withhold the
  // document itself.
  if (mode === 'visual-judge') {
    if (!process.env.GEMINI_API_KEY) {
      return res.status(501).json({ error: 'Visual check unavailable: GEMINI_API_KEY is not configured.' });
    }
    try {
      const result = await callGeminiVision(process.env.GEMINI_API_KEY, VISUAL_JUDGE_PROMPT, imageBase64, VISUAL_JUDGE_TOKENS);
      return res.status(200).json({ content: result.text, modelUsed: result.modelUsed });
    } catch (err) {
      return res.status(500).json({ error: err.message || 'Visual check failed.' });
    }
  }

  let chatMessages;
  if (mode === 'plan') {
    // OPTIONAL — only runs if SERPER_API_KEY is configured (get a free key
    // at serper.dev; without it this is a silent no-op and plan generation
    // uses the built-in board/subject patterns in PLAN_PROMPT as before).
    // When configured, this looks up the REAL current format for the
    // exact board/class/subject asked for before planning the paper, so
    // markScheme is grounded in an actual current pattern rather than the
    // model's best guess from training data.
    const searchContext = /paper|exam|specimen|test|sample paper|question paper/i.test(userRequest)
      ? await searchPaperFormatIfConfigured(userRequest)
      : '';
    chatMessages = [
      { role: 'system', content: PLAN_PROMPT },
      { role: 'user', content: userRequest + searchContext },
    ];
  } else if (mode === 'summary') {
    chatMessages = [
      { role: 'system', content: SUMMARY_PROMPT },
      { role: 'user', content: userRequest.slice(0, 12000) },
    ];
  } else if (mode === 'judge') {
  ...
  const isHeavySection = mode === 'section' && (questionCount > 3 || /answer key/i.test(sectionHeading || '') || type === 'ebook');
  const isAnswerKeySection = mode === 'section' && /answer key/i.test(sectionHeading || '');
  const dynamicTokens = mode === 'judge' ? JUDGE_TOKENS : mode === 'summary' ? SUMMARY_TOKENS : (isHeavySection ? HEAVY_SECTION_TOKENS : TOKENS_PER_CALL);
  const chain = (mode === 'judge' || isAnswerKeySection) ? QUALITY_PROVIDER_CHAIN : (CHAIN_BY_TIER[tier] || MID_TIER_CHAIN);

  try {
    const { text, finishReason, modelUsed, providerUsed } = await callWithFallback(chain, chatMessages, dynamicTokens, req.headers.origin, tier);
    return res.status(200).json({ content: text, finishReason, modelUsed, providerUsed, tier });
  } catch (err) {
