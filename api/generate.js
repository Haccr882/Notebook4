...
import { checkPremiumToken, checkAndIncrementLimit, spendCredit, resolveIdentity } from '../lib/_limits.js';
...
//
// TWO different orderings on purpose (see PROVIDER_DAILY_LIMITS in
// lib/providers.js for the budget behind this): ordinary plan/section
// writing goes through the CHEAP chain — fast, high-volume, free models
// first, since most sections are routine and any of these do a perfectly
// good job. The QUALITY chain instead leads with Gemini — the
// deliberately protected "powerful model" reserve (600/day, floor
// 500+) — for the two places a wrong/mismatched result actually hurts a
// student: the judge/visual-judge quality-check passes, and Answer Key
// sections (which must genuinely match the real questions, not just
// read plausibly).
const CHEAP_PROVIDER_CHAIN = ['openrouter', 'grok', 'groq', 'gemini', 'nvidia', 'openrouterFallback'];
const QUALITY_PROVIDER_CHAIN = ['gemini', 'grok', 'groq', 'openrouter', 'nvidia', 'openrouterFallback'];
const SECTION_PROVIDER_CHAIN = CHEAP_PROVIDER_CHAIN;
const PLAN_PROVIDER_CHAIN = CHEAP_PROVIDER_CHAIN;
const JUDGE_PROVIDER_CHAIN = QUALITY_PROVIDER_CHAIN;
const SUMMARY_PROVIDER_CHAIN = CHEAP_PROVIDER_CHAIN; // low-stakes, on-demand — no reason to spend the protected reserve here
...
Depth and word count should genuinely match what a real published ebook chapter on this exact subject needs — this is the one type where being SUBSTANTIAL is correct, unlike notes (which stay deliberately tight).`;
  } else {
...
export default async function handler(req, res) {
  ...
  const chain = mode === 'plan' ? PLAN_PROVIDER_CHAIN
    : mode === 'judge' ? JUDGE_PROVIDER_CHAIN
    : mode === 'summary' ? SUMMARY_PROVIDER_CHAIN
    : isAnswerKeySection ? QUALITY_PROVIDER_CHAIN
    : SECTION_PROVIDER_CHAIN;
