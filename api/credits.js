...
import { getCreditBalance, resolveIdentity, checkPremiumToken, PLANS } from '../lib/_limits.js';
...
export default async function handler(req, res) {
  ...
  const isPremium = await checkPremiumToken(premiumToken); // legacy UPI+redeem-code path — still supported alongside plans
  if (isPremium) {
    return res.status(200).json({ premium: true, plan: 'premium', signedIn: !!user });
  }
