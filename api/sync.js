...
import { verifyGoogleIdToken, verifySession } from '../lib/_limits.js';

const MAX_PAYLOAD_BYTES = 900 * 1024; // Upstash free tier caps request/value size — stay comfortably under it

async function requireUser(req){
  const { googleIdToken, sessionToken } = req.body || {};
  const sToken = sessionToken || (req.query && req.query.sessionToken);
  if (sToken) {
    const user = verifySession(sToken);
    if (user) return user;
  }
